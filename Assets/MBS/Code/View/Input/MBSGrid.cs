    ...
    public static class MbsGrid
    {
        ...
        private static void CalcLinesPoints( )
        {
            ...
            for ( var i = 0; i < halfCapacity - 1; i++ )
            {
                ...
                {
                    lineWidth = 3;
                    lineColor = new Color( .6f, .6f, .6f, 0.7f );
                }
                else
                {
                    lineWidth = 2;
                    lineColor = new Color( .5f, .5f, .5f, 0.2f );
                }
        ...
        private static void DrawLines( )
        {
            for ( var i = 0; i < _gridLines.Count; i++ )
            {
                ...
                Handles.DrawAAPolyLine( _gridLines[ i ].points );
            }
        }
        ...
        public static Vector3 Snap_ToCenter( Vector3 pos_Constr )
        {
            ...
            return new Vector3(
                Mathf.Floor( ( pos_Constr.x - _offsetPosition.x ) / CellSize ) * CellSize + halfCell + _offsetPosition.x,
                LevelHeight * LevelNumber,
