...
{
    [Serializable]
    internal class RoomArea : GeometryShape
    {
        [SerializeField] internal List<Vector3> wallEndPoints;
        [SerializeField] internal List<MBSWallModule> walls;
        [SerializeField] internal Vector2 areaBounds;
        [SerializeField] private float _height;
        [SerializeReference] internal List<Triangle> triangles;
                         ...
                         bool doFaceElements = true )
        {
            ...
            // if ( doFaceElements )
            //     FaceAreaInside( walls[ 0 ].MbsConstruction );
        }


        internal List<MBSWallModule> Walls
        {
            get
            {
                if ( walls == null )
        ...
        internal (float area, WindingOrder windingOrder) FindAreaAndWindingOrder( List<Vector3> areaExtremePoints )
        {
            var area = 0f;
            WindingOrder order;
        ...
        {
            var asd = Walls.Select( i => i.gameObject ).ToArray( );
            for ( var i = 0; i < asd.Length; i++ )
            {
                EditorGUIUtility.PingObject( asd[ i ].gameObject );
            }
        }
            ...
            for ( var i = 0; i < retval.Count; i++ )
                for ( var j = 0; j < retval.Count; j++ )
                {
                    ...
                    if ( intersectedPoints.Length > 0 && intersectedPoints.Length < 3 )
                    {
                        ...
                        {
                            //need to change to crossproduct
        ...
        public bool IsPointInsideArea( Vector3 pointer )
        {
            ...
            //Point inside area cause it is on the edge of the area (on the path)
            ...
            //Check each triangle in area is point inside it or not
            for ( var i = 0; i < triangles.Count; i++ )
            {
                ...
                if ( checker.isInside )
                {
                    //if point inside triangle, increase @insideNumber
                    insideNumber++;

                    //if point not only inside, but on the edge then increase @edgeNumber variable and add triangle to the @trisOnEdge list
                    if ( checker.isEdge )
                    {
                        edgeNumber++;
            ...
            //if @insideNumber and edgeNumber more than one
            //which means that point inside two or more triangles, and lais on the edges
            //we need to check intersection between these triangles
            //because if point lais on the edge of two triagles that intersect with each other
            //we should not count it as two inputs, but as one
        ...
        public void FaceAreaInside( MBSConstruction mbsConstruction )
        {
            if ( Walls == null || Walls.Count == 0 )
                return;

            if ( Walls.All( i => i.IsFacesInsideArea( this ) ) )
                return;

            //Undo.IncrementCurrentGroup( );
            //Undo.SetCurrentGroupName( "MBS. Area Flip Inside." );
            //construction.RecordObjectUndo( );


            for ( var i = 0; i < Walls.Count; i++ )
                Walls[ i ].FaceWallInsideArea( this );
