...
namespace MBS.Controller.Scene
{
    ...
    internal class Decorator_PlacementMode
    {
        internal static readonly string[ ] PLACEMENT_TYPE_TEXT = { "All Surfaces", "Upward Plus", "Upward Only" };

        public static readonly string[ ] PLACEMENT_FACING_TEXT =
            { "None", "X Axis", "Y Axis", "Z Axis", "-X Axis", "-Y Axis", "-Z Axis" };

        // general
        internal static PlacementType placementType;
        internal static PlacementFacing placementFacing;
        internal static bool DoSnapToGrid;
        internal static bool IsCollisionOn;
        internal static bool RandomModuleOnPlace;
        internal static bool LimitHeightByMBSLevel;
        internal static float DefaultRaycastDistance;
        internal static int RaycastOffsetCoefficient;

        // non-alloc raycast vars
        ...
        internal static void Run( Transform transform, GameObject originalPrefab, Quaternion currentRotation,
        ...
        internal static void ChangePlacementType( PlacementType placementType )
        {
            Decorator_PlacementMode.placementType = placementType;
        }

        internal static void ChangePlacementType_Next( )
        {
                                                      ...
                                                      Bounds localBounds, Bounds worldBounds )
        {
            ...
            // Check if new position have overlaps
            var overlapNumber =
            ...
            // If there was an overlaps
            // Recalculate new position with BoxCast
            ...
            {
                // physicsOffset = rayHit.normal * Physics.defaultContactOffset;
                ...
                {
                    return newCenterPos - _pivotOffset - physicsOffset +
                           ( velocityNormal * Physics.defaultContactOffset );
                }
            }
        ...
        private static Vector3 AllSurfaces_NoCollision( RaycastHit rayHit, Quaternion rotation, Bounds localBounds, Bounds worldBounds )
        {
            var physicsOffset = rayHit.normal * Physics.defaultContactOffset;
            var newCenterPos = rayHit.point
                             + rayHit.normal* LocalExtentNormalDot( rayHit.normal, localBounds.extents, rotation )
                                                                ...
                                                                Bounds localBounds, Bounds worldBounds )
        {
            ...
            // means we need to calculate collision effect
            // and recalculate new position
            //prevCenterPosition = prevCenterPosition - ( velocityNormal * velocityMagnitude );
            ...
            // if non of the above conditions occured
            // back off a little
            return currentObjectPos - ( velocityNormal * Physics.defaultContactOffset );
        }
                                                                 ...
                                                                 ref Vector3 prevPosition )
        {
            ...
            if ( overlapNumber > 0 )
            {
                // If there are more than 0 overlaps
                // Recalculate new_TopPosition with castBox
                                      ...
                                      _castBoxHitNonAlloc, out var boxHit1 ) )
                {
                    ...
                    else
                    {
                        ...
                        var j = ( ( -( 1 + 1 ) * Vector3.Dot( velocity, hitNormal ) ) / 2 );
                        // TODO: need to add physics default contact offset?
                        var v2 = velocity + j * hitNormal;
