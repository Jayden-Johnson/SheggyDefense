...
{
    internal static class Decorator_ScalingMode
    {
        internal enum ScalingAxis { All, XAxis, YAxis, ZAxis }

        internal enum ScalingSnapAccuracy { OneTenth, OneHundredth, OneThousands }

        internal static string[ ] ScalingAxisText_Array = { "All", "X Axis", "Y Axis", "Z Axis" };
        internal static float[ ] ScalingSnapAccuracy_ValuesArray = { 0.1f, 0.01f, 0.001f };
        private static string[ ] _guiValueRound = { "F1", "F2", "F3" };

        internal static ScalingAxis Axis;
        internal static ScalingSnapAccuracy SnapAccuracy;

        internal static Vector3 initialScale;
        internal static Vector3 currentScale;


        internal static void Start( Transform transform )
        {
            Clear( );
        ...
        internal static void Run( Transform transform )
        {
            ...
            Handles.BeginGUI( );
            {
                ...
                // Rect rect = new Rect(e.mousePosition.x, e.mousePosition.y, 140, 20);
        ...
        internal static Vector3 Accept( Transform transform )
        {
            Clear( );
            return  transform.localScale;
        }

        internal static void Cancel( Transform transform )
        {
            Reset( transform );
            Clear( );
        }
