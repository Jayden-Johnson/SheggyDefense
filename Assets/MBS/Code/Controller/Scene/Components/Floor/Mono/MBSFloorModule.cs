...
namespace MBS.Controller.Scene.Mono
{
    ...
    [ExecuteInEditMode]
    internal class MBSFloorModule : MonoBehaviour
    {
        [Serializable]
        internal class FloorAssetData
        {
            [SerializeField] internal string packGuid;
            [SerializeField] internal string groupGuid;
            [SerializeField] internal string moduleGuid;
            [SerializeField] internal FloorTileType tileType;
            [SerializeField] internal FloorCorner corner;
        }
        ...
        internal FloorAssetData Data => _data;

        internal ModularPack OrigPack
        {
            get
            {
                if ( _origPack != null )
        ...
        internal FloorModule OrigModule
        {
            get
            {
                if ( _origGroup != null )
        ...
        private void LoadAssetsByGUIDs( )
        {
            ...
            if ( _origModule == null )
            {
                Debug.Log( "Module guid: " + Data.moduleGuid );
                foreach ( var a in OrigGroup.Modules ) Debug.Log( "List item: " + a.Guid );
                Debug.LogError( "MBS. Wall Item. Cannot initialize data, module with given GUID is not found." );
            }
        }


        internal MBSFloorModule ChangeModule( FloorModule newModule, bool isMultipleChange )
        {
        ...
        internal void WhenPlaced( ModularPack pack, ModularGroup group, Module module, FloorTileType tileType,
                                  FloorCorner corner )
        {
            _origPack = pack;
