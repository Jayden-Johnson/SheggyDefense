...
{
    internal class WallPrefabContoller
    {
        ...
        internal static (GameObject chosenPrefab, Vector3 additionalScale) ChangeModule_ChosePrefab(
            WallModule prevModule, WallModule newModule, GameObject prevOrigPrefab, float additionalScale )
        {
            ...
            if ( prevOrigPrefab == prevModule.DefaultPrefab )
            {
                ...
                {
                    // If prefab scale is more than MaxLimit
                    // It is better to use shrink Stretched prefab
                    if ( newModule.ExtendedPrefab != null )
                    {
