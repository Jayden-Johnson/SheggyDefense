...
{
    internal enum WallAngleSnapStep
    {
        OneTenth,
        One,
        Ten,
    ...
    internal static class BuildingProcedures_Walls
    {
        #region AssetVars
        ...
        private static Vector3 _currentPrefabSize;
        ...
        internal static WallAngleSnapStep SnapStep;
        internal static float[ ] SnapStepValueArray = { 0.1f, 1.0f, 10.0f, 15.0f, 90.0f };

        internal static int levelsNumber = 1;

        #endregion


        internal static void StartDrawing( )
        {
        ...
        internal static void EndDrawingWithoutSaving( )
        {
            SceneData.IsMouseRecalcNeeded = true;
            EndWithoutSaving( );
            InputWalls_Tool.Setup_IdleInputs( );
        ...
        internal static bool Start( )
        {
            ...
            _currentPrefabSize = _defaultPrefabSize;
        ...
        internal static Vector3 Building( Vector3 currentPosConstr )
        {
            SceneData.DoStretchWalls = true;


            //Handles.SphereHandleCap( 0, MBSConstruction.Current.transform.TransformPoint( Mouse.FreeConstrPos ) , Quaternion.identity, 0.1f, EventType.Repaint );
        ...
        {
            // using @additinalScale to stretch wall to fit 45 degree 
            // using Is45DegreeDrawing to indicate that the wall is stretched
            ...
            else
            {
                ...
                else
                {
                    ...
                    _currentPrefabSize = _extendedPrefabSize;
                }
            }

            itemsFit = deltaPos.magnitude / ( _currentPrefabSize.x * _extendScale );
            ...
            var scaledSize = _currentPrefabSize.x * _extendScale;
            ...
            for ( int i = 0; i < _currentDrawingItems.Count; i++ )
            {
                for ( var j = 0; j < _currentDrawingItems[ i ].Count; j++ )
                {
                    _currentDrawingItems[ i ][ j ].transform.localPosition =
                        Vector3.right * ( scaledSize * j + scaledSizeHalf ) + Vector3.up * MbsGrid.LevelHeight * i;
        ...
        {
            // using @fitScale to stretch wall to fit it between end points
            ...
            var scaledSize = _currentPrefabSize.x * _fitScale;
            var scaledSizeHalf = scaledSize / 2;
        ...
        internal static void End( )
        {
            ...
            else
            {
                ...
                var modularPack = ModularPack_Manager.Singleton.ModularPacks.ToList( )
                                                     .Find( i => i.WallGroups.Contains(
                                                                BuilderDataController.SelectedGroup ) );
                ...
                for ( var i = 0; i < _currentDrawingItems.Count; i++ )
                {
                    for ( var j = 0; j < _currentDrawingItems[ i ].Count; j++ )
                    {
                        ...
                        // instantiate mesh for each 2 walls
                        // because each calculation propagate onto neightbors
            ...
            _currentPrefabSize = Vector3.zero;
