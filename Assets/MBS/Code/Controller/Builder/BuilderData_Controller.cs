...
namespace MBS.Controller.Builder
{
    internal static class BuilderDataController
    {
        ...
        internal static ToolbarTool Tool => _data.SelectedTool;
        internal static ToolData ToolData => _data.SelectedToolData;
        ...
        // Create/load BuilderData scriptableObject
        // Create DataContainer for each toolbar tool
        // Combine packs -> create "All" modular pack
        // Call SelectTool(Wall) so the initial assets could load
        ...
        private static void InitializeBuilderData( )
        {
            var builderDataPath = PathController.GetPATH_BuilderDataAsset( );
            _data = AssetDatabase.LoadAssetAtPath<BuilderData>( builderDataPath );

            if ( _data == null )
            {
                _data = ScriptableObject.CreateInstance<BuilderData>( );
                _data.ColdInitialization( );
                AssetDatabase.CreateAsset( _data, builderDataPath );
            }
        }

        private static void CombinePacks( )
        {
            var packs = ModularPack_Manager.Singleton.DescriptorsData.Where( i => !i.IsHidden ).Select( i => i.Pack )
                                           .ToArray( );
        ...
        private static void LoadToolsAssets( )
        {
            ...
            {
                // If ModularPack by guid is null
                // Try to get first ModularPack
                loadedModularPack = GetModularPackByIndex( 0 );

                // If it doesnt work
                // Then flush and return
                // We cant get AssetGroup without ModularPack
                if ( loadedModularPack == null )
                {
            ...
            // If ModularPack is not null
            // Set index, guid, reference
            SetupModularPack( loadedModularPack, AllModularPacks );

            // If ModularPack recieved
            // Get the AssetGroup by GUID
            var loadedModularGroup = GetModularGroupByGuid( _data.SelectedToolData.GroupGuid );
            if ( loadedModularGroup == null )
            {
                // If AssetGroup by GUID is null
                // Try to get first AssetGroup
        ...
        internal static void SaveBuilderData( )
        {
            if ( _data != null )
                AssetDatabase.SaveAssetIfDirty( _data );
        }

        internal static void SelectTool( ToolbarTool tool )
        {
            _data.SelectedTool = tool;
            LoadToolsAssets( );
        }

        internal static bool SelectModularPackAt( int index )
        {
        ...
        internal static bool SelectAssetGroupAt( int index )
        {
        ...
        public static void SelectRandomModule( )
        {
            ...
            if ( SelectedGroup.CastedModules is null || SelectedGroup.CastedModules.Length == 0 || SelectedGroup.CastedModules.Length == 1 )
                return;
