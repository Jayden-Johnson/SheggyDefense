...
{
    [Serializable]
    internal class Module : UniqueNamedObject
    {
        ...
        internal Texture2D GetPreviewOrEmptyIcon( )
        {
        ...
        internal virtual Vector3 GetSize( int index = 0 )
        {
            var mesh = GetPrefabCombinedMeshAt( index );

            if ( mesh == null )
                //Debug.LogError( "Cannot get prefab size. There is no mesh attached to the prefab and its children." );
                return Vector3.one;

            return mesh.bounds.size;
        }

        internal static Vector3 GetSize( Module module, int index = 0 )
        {
            if ( module == null )
                return Vector3.one;
        ...
        internal Mesh GetPrefabCombinedMeshAt( int prefabIndex )
        {
            if ( prefabs == null || prefabs.Length == 0 )
                return null;
        ...
        internal static Texture2D GetPreviewOrEmptyIcon( Module asset )
        {
            if ( asset == null )
    ...
    [Serializable]
    internal class WallModule : Module
    {
        internal WallModule( )
        {
        ...
        internal GameObject DefaultPrefab
        {
            get => prefabs[ 0 ];
            set => prefabs[ 0 ] = value;
        }

        internal GameObject ExtendedPrefab
        {
            get => prefabs[ 1 ];
            set => prefabs[ 1 ] = value;
        }
    ...
    [Serializable]
    internal class FloorModule : Module
    {
        public FloorModule( )
        {
            prefabs = new GameObject[ 2 ];
    ...
    [Serializable]
    internal class DecoratorModule : Module
    {
